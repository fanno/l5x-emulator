...
class XIO(Instruction):
    ...
    async def ladder_execute(self, ctx:"ExecutionContext") -> None:
        if ctx.RLL.RungStatus:
            ctx.RLL.RungStatus &= not bool(self.getMemory(self.args[0]))

@InstructionRegistry.register
class OTE(Instruction):

    async def ladder_execute(self, ctx:"ExecutionContext") -> None:
        self.setMemory(self.args[0], ctx.RLL.RungStatus)
...
class ONS(Instruction):
    ...
    async def ladder_execute(self, ctx:"ExecutionContext") -> None:
        ...
        ons.setValue(ctx.RLL.RungStatus)
        if ons and ctx.RLL.RungStatus:
